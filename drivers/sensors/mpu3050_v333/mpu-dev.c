...
int read_accel_raw_xyz(struct acc_data *acc)
{
	...
	if (mldl_cfg->accel_is_suspended == 1 ||
		...
		if (sensor_i2c_read
		    (this_client->adapter, 0x0F, 0x06, 6, acc_data) != 0) {
			return -1;
		}
		...
		if (sensor_i2c_read
		    (this_client->adapter, DEFAULT_MPU_SLAVEADDR, 0x23, 6,
		     acc_data) != 0) {
			return -1;
		}
	} else
	...
	case MPU_READ_PRESSURE:
		{
			...
			struct i2c_adapter *pressure_adapt =
			    i2c_get_adapter(mldl_cfg->pdata->
					    pressure.adapt_num);
			retval =
				...
				struct device_attribute *attr, char *buf)
{
	...
	printk(KERN_DEBUG "read temperature = %d\n", temperature);
...
static ssize_t p3_accel_cal_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t size)
{
	cal_data.x = (s8)buf[0] * (-1);
	cal_data.y = (s8)buf[1] * (-1);
	cal_data.z = (s8)buf[2] * (-1);

	printk(KERN_INFO "accel cal : %d, %d, %d\n",
		cal_data.x, cal_data.y, cal_data.z);
